...
inline
void combine(Sales_data &current, Sales_data &other)
{
...
const std::string &isbn(Sales_data &item) { return item.book_isbn; }
...
int main()
{
    ...
    if (read(std::cin, total))
    {
        ...
        {
            if (isbn(total) == isbn(transaction))
            {
                #ifndef NDEBUG
                std::cout << __LINE__ << ": Matching ISBNs" << std::endl;
                #endif
                combine(total, transaction);
