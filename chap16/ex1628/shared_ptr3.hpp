...
template <typename Type>
class control_block_internal: public control_block_base<Type>
{
    friend shared_ptr<Type>;
public:

    template <typename... Args>
    control_block_internal(Args&&... args):
        internal_pointee(std::forward<Args>(args)...)
    {TRACE();}

private:
    Type internal_pointee;
};
...
private:
    ...
    explicit shared_ptr(control_block_internal<Type>* cb):
        pointee(&cb->internal_pointee),
        cb(cb)
    {}
...
{
    return shared_ptr<T> (new control_block_internal<T>(std::forward<Args>(args)...));
}
