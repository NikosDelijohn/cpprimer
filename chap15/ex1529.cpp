...
int main()
{
    ...
    std::vector<std::pair<size_t, std::shared_ptr<Quote>>> basket;
    
    basket.emplace_back(4, std::make_shared<Bulk_Item>("123", 15.0, 5, .30));
    basket.emplace_back(100, std::make_shared<Bulk_Item>("456", 1.0, 100, .20));

    double total_due = 0.0;
    
    for(auto &item: basket)
    {   
        print_total(std::cout, *item.second, item.first);
        total_due += item.second->net_price(item.first);
    }
    
    std::cout << "Total amount due for purchases is: " << total_due << " €\n";
